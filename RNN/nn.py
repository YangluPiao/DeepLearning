    ...
    def __init__(self, nh, nc, ne, de, cs, normal=True,layer_norm=False):
        ...
        if layer_norm:
            ...
            def recurrence(x_t, h_tm1):
                a_t = T.dot(h_tm1, self.wh)+T.dot(x_t, self.wx)
                miu_t = T.mean(a_t)
                seta_t = T.sqrt(T.mean((a_t-miu_t)**2))
                h_t = T.nnet.sigmoid((self.g/seta_t)*(a_t-miu_t)+self.bh)
...
def train_nn(train_model, validate_model, test_model, n_train_batches, n_valid_batches, n_test_batches, n_epochs, fil, verbose = True):
    """
    ...
    patience = 150000  # look as this many examples regardless
    ...
    print('Optimization complete.',file=f)
